...
var pageWidth = 0, pageHeight = 0, pieHeight = 0, rightWidth = 0, lTicket = false, nSize, adicionalkude;
async function generarKude(data, res, params, kuDeBinary) {
    let lLogo = false;
...
async function cabecera(doc, data, cRuc, params, lLogo) {
...
function detalle(doc, data, nSize) {
    ...
    data.items.forEach((item) => {
        ...
        if (item) {
            ...
            position = generateTableRow(doc, position, item.dCodInt, item.dCantProSer, item.dDesProSer, currency(item.dPUniProSer, data.cMoneOpe), currency((+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt), data.cMoneOpe), currency((item.dTasaIVA == 0) ? item.dCantProSer * (item.dPUniProSer - (+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt)) : 0, data.cMoneOpe), currency((item.dTasaIVA == 5) ? item.dCantProSer * (item.dPUniProSer - (+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt)) : 0, data.cMoneOpe), currency((item.dTasaIVA == 10) ? item.dCantProSer * (item.dPUniProSer - (+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt)) : 0, data.cMoneOpe), nSize);
...
function detalleTicket(doc, data, lTicketResumen) {
    ...
    data.items.forEach((item) => {
        if (item) {
            if (lTicketResumen == false) {
                generateTableRowTicket(doc, item.dCodInt, formatDecimal(item.dCantProSer), item.dDesProSer, currency(item.dPUniProSer, data.cMoneOpe), currency((+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt), data.cMoneOpe), formatDecimal(item.dTasaIVA), currency((item.dCantProSer * (item.dPUniProSer - (+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt))), data.cMoneOpe));
            }
            exentas += ((item.dTasaIVA == 0 ? item.dCantProSer * (item.dPUniProSer - (+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt)) : 0));
            gravada5 += ((item.dTasaIVA == 5 ? item.dCantProSer * (item.dPUniProSer - (+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt)) : 0));
            gravada10 += ((item.dTasaIVA == 10 ? item.dCantProSer * (item.dPUniProSer - (+item.dDescGloItem + +item.dAntGloPreUniIt + +item.dDescItem + +item.dAntPreUniIt)) : 0));
...
function totalTicket(doc, titulo, monto, moneda) {
    let y = doc.y;
    doc.text(titulo, leftMargin + 20, y).text(currency(monto, moneda), pageWidth - 120, y, { width: 100, align: "right" });
}
function generateTableRow(doc, y, codigo, cantidad, descripcion, costoUnit, descuento, exentas, gravadas5, gravadas10, nSize) {
    doc.text(codigo, leftMargin + 5, y)
...
function detalleAF(doc, data) {
    ...
    var position = invoiceTableTop + 20; // guardo solo la posicion inicial (primer item)
    data.items.forEach((item) => {
        position = lineaAF(doc, position, item.dCodInt, item.dCantProSer, item.dDesProSer, currency(item.dPUniProSer, data.cMoneOpe), currency(item.dCantProSer * item.dPUniProSer, data.cMoneOpe));
...
function detalleEscribania(doc, data, totalFactura, nSize) {
...
async function redimensionaTicket(inputPath, outputPath) {
